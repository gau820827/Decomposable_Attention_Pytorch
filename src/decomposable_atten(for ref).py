...
class DecomposableAttention(nn.Module):
    ...
    def __init__(self, embedding_dim, hidden_dim, output_dim, 
                 loaded_embeddings, mode='glove', vocab_size=0):
...
def train_iter(model, data_iter, iter_time, pt):
    """Training loop."""
    optimizer = optim.Adam(model.parameters())
    lossf = nn.CrossEntropyLoss()


    data_iter_vlaidation = pt.batch_iter(pt.matrix_validation, pt.batch_size, usepretrain, reuse)
...
def train(embedding_dim, hidden_dim, output_dim, batch_size,
    ...
    if usepretrain:
        ...
        data_iter = pt.batch_iter(pt.matrix_train, pt.batch_size, usepretrain, reuse)
        test = next(data_iter)
    ...
    train_iter(model, data_iter, 100,pt)
...
if __name__ == '__main__':
    ...
    # pretrain_filename = 'test_glove.txt'
    ...
    train(embedding_dim, hidden_dim, output_dim, batch_size, usepretrain,
          pretrain_emb_size, pretrain_filename, document_filename, ignore, reuse)
